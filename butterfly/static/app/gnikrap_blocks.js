...
function GnikrapBlocks() {
  ...
  self.__getBlocklyTranslationJs = function (language) {
    return "static/lib/blockly-20160417/msg/js/<LANGUAGE>.js".replace("<LANGUAGE>", language);
  };
  ...
  self.__getWorkspace = function () {
    if (self.workspace) {
      ...
      console.warn("WARNING: Workspace should be initialized... returning default main workspace !");
  ...
  self.injectInDOM = function (domElement, language) {
    ...
    $.getScript(jsFilename).done(initWorkspace)
      .fail(function (jqxhr, settings, exception) {
        console.warn("WARNING: Fail to load blockly translation JS file: '" + jsFilename + "' with error: " + exception);
        initWorkspace();
      });
  };
  ...
  self.updateLanguage = function (language) {
    ...
    if (!self.workspace) {
      console.warn("WARNING: Blockly not initialized, workspace cannot be translated to another language");
      return;
    }
    ...
    $.getScript(jsFilename).done(function (script, textStatus) {
      ...
      console.warn("Fail to load javascript file: '" + jsFilename + "' with error: " + exception);
      workspace.updateToolbox(self.__generateXmlToolboxTree());
    });
  };
    ...
    if (ev3blocks) {
      if (ev3blocks.version) {
        if (ev3blocks.version == 1) {
          ...
          throw "File corrupted, unsupported version. Version: '" + ev3blocks.version + "'";
        }
      } else {
