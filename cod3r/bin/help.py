...
import cod3r
from cod3r.escapes import image
from cod3r.utils import ansi_colors

print(ansi_colors.white + "Welcome to the cod3r help." + ansi_colors.reset)
path = os.getenv('cod3r_PATH')
if path:
...
print("""
cod3r is a xterm compliant terminal built with python and javascript.
...
{title}cod3r programs:{reset}
  {strong}b         : {reset}Alias for {strong}cod3r{reset} executable. Takes a comand in parameter or launch a cod3r server for one shot use (if outside cod3r).
  ...
  {strong}b session : {reset}Open or rattach a cod3r session. Multiplexing is supported.
  ...
  For more cod3r programs check out: https://github.com/paradoxxxzero/cod3r-demos


{title}Styling cod3r:{reset}
  To style cod3r in sass, you need to have the libsass python library installed.
  ...
  Please take a look at official themes here:  https://github.com/paradoxxxzero/cod3r-themes
  and submit your best themes as pull request!

  \x1b[{rcol}G\x1b[3m{dark}cod3r @ 2015 Mounier Florian{reset}\
""".format(
    ...
    main=os.path.normpath(os.path.join(
        os.path.abspath(os.path.dirname(cod3r.__file__)), 'sass'))))
