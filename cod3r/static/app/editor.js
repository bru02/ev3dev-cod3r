...
class DoubleEditor {
    constructor(tag, settings) {
        ...
        var editor = this, firstEdit = true;
    ...
    showTextMenu() {
        ...
        // Resize sidebar
        let codemirrorGutterWidth = $(".ace-gutters").width(), sideBarWidth = this.blocklyToolboxWidth - codemirrorGutterWidth - 2;
    ...
    setBlocks(python_code) {
        ...
        let error_code = this.converter.convertSourceToCodeBlock(python_code), errorXml = Blockly.Xml.textToDom(error_code);
    ...
    refreshBlockHighlight(line) {
        ...
        if (1 + line in blockMap) {
            let hblocks = blockMap[1 + line], blockly = this.blockly;
     ...
     */
    updateToolbox(only_set) {
        var xml = "<xml id=\"toolbox\" style=\"display: none\">", modules = ["Calculation", "Output", "Values"], started_misc = false, started_values = false, started_data = false;
