...
function newBlock(name) {
  ...
  let block = blockpy.components.editor.blockly.newBlock(name);
  block.initSvg();
  block.render();
...
Blockly.Xml.domToWorkspaceDestructive = function (xml, workspace, errorXml) {
  ...
  let childCount = xml.childNodes.length,
  ...
  for (let i = 0; i < childCount; i++) {
    ...
    let xmlChild = xml.childNodes[i],
      name = xmlChild.nodeName.toLowerCase();
    if (name == "block" ||
      ...
      let block = Blockly.Xml.domToBlock(xmlChild, workspace),
...
function PLUS_MINUS_updateShape(listItemName, startMessage) {
  ...
  return function () {
    ...
    function addField(field, block, e) {
      ...
      let rect = field.fieldGroup_.getBoundingClientRect(),
        yPosition = e.clientY;
      if (yPosition < rect.top + rect.height / 2) {

        let input = that.appendValueInput(listItemName + that.itemCount_);
        that.itemCount_ += 1;

      } else if (that.itemCount_ > 0) {
        that.itemCount_ -= 1;
    ...
    if (!this.getInput("START")) {
      ...
      let clickablePlusMinus = new Blockly.FieldClickImage("images/plus-minus-button.svg", 12, 24, "+", addField, "-2px");
