...
function DoubleEditor(tag, settings) {
    ...
    settings.editor.subscribe(() => { editor.setMode() });
...
DoubleEditor.prototype.initText = function () {
    ...
    this.codeMirror = ace.edit("aceEditor");
    // Register model changer
    let editor = this;
    this.codeMirror.getSession().on("change", () => {
        //editor.main.components.feedback.clearEditorErrors();
        editor.updateText()
    ...
    this.codeMirror.setSize = function (w, h) {

        editor.codeMirror.container.style.height = h;
        editor.codeMirror.container.style.width = w;
...
DoubleEditor.prototype.setCodeLang = function (lang) {
    ...
    this.codeMirror.session.setMode("ace/mode/" + lang);
...
DoubleEditor.prototype.updateBlocks = function () {
    ...
    if (!this.silenceBlock) {
        ...
        // Update Model
        this.silenceModel = 2;
        let changed = false;  //this.main.setCode(newCode);
        if (!changed) {
...
DoubleEditor.prototype.refreshHighlight = function () {
    ...
    if (this.previousLine !== null) {
        ...
        if (this.previousLine < this.codeMirror.session.doc.$lines.length) {

            this._error = editor.session.hightlightLines(this.previousLine, this.previousLine, "editor-error-line");
...
DoubleEditor.prototype.highlightLine = function (line) {
    ...
    if (this.previousLine !== null) {
        ...
        if (this.previousLine < this.codeMirror.session.doc.$lines.length) {
            ...
            if (this._error) { this.codeMirror.session.removeMarker(this._error), this._error = null; }
            if (this._active) { this.codeMirror.session.removeMarker(this._active), this._active = null; }

        }

    }
    if (line < this.codeMirror.session.doc.$lines.length) {

        this._active = editor.session.hightlightLines(this.previousLine, this.previousLine, "editor-active-line");
...
DoubleEditor.prototype.highlightError = function (line) {
    ...
    if (line < this.codeMirror.session.doc.$lines.length) {
        ...
        this._error = this.codeMirror.session.hightlightLines(this.previousLine, this.previousLine, "editor-error-line");
